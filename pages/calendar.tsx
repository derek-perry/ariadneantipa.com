    ...
    () => {
      ...
      const getPastEvents = async () => {
        try {
          ...
          const fetchedData = [];
          const { data } = await api.get(
            `events?pagination[page]=1&pagination[pageSize]=10&sort[0]=id:desc&populate[Day][fields][4]=StartTime&populate[Day][fields][5]=EndTime&populate[Day][fields][6]=Price&populate[Day][populate][0]=Timezone&populate[Day][populate][1]=Location`
          );
          fetchedData.push(...data?.data);
          if (
            data?.meta?.pagination &&
            ...
            for (let i = page + 1; i <= pageCount; i++) {
              let response = await api.get(
                `events?pagination[page]=${i}&pagination[pageSize]=10&sort[0]=id:desc&populate[Day][fields][4]=StartTime&populate[Day][fields][5]=EndTime&populate[Day][fields][6]=Price&populate[Day][populate][0]=Timezone&populate[Day][populate][1]=Location`
              );
              fetchedData.push(...response.data.data);
            };
          };
        ...
        {isLoadingPastEvents ? (
          ...
          (pastEvents && pastEvents.length ? (
            ...
            >
              <hr className='hrFancy max-w-[600px] !mt-0' />
              {(pastEvents.length > 1 ? (
                <h2 className='my-8' id='past-events'>Past Events</h2>
              ) : (
                <h2 className='my-8' id='past-event'>Past Event</h2>
              ))}
              <div
                className='w-full flex flex-col gap-6 justify-center align-middle items-center text-center'
                id='past-events-list'
